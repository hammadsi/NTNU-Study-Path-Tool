...
def get_subject_name(user_email,user_course_name,formatted_courses):
    ...
    def subject_Ai_model():
        # Define a new Pydantic model with field descriptions.
        class CourseUser(BaseModel):
            subjects: str = Field(description=f"Which subject user will take from this : {course_name}")
        ...
        prompt = ChatPromptTemplate(
            messages=[
                HumanMessagePromptTemplate.from_template(
                    "Based on the user's answers, recommend a subject of course from the following options: "
